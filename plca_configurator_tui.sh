...
while [ "${returncode:-99}" -ne 1 ] && [ "${returncode:-99}" -ne 250 ]; do
	...
	value=`dialog \
	      ...
	      --inputmenu "PLCA Configurator" 30 65 55 \
	      "1. Interface Name (Ex: eth1)"			"$iface_name" \
	      "2. PLCA Mode (on/off)"				"$plca_mode" \
	      "3. PLCA Node ID (0-254)"				"$plca_node_id" \
	      "4. PLCA Node Count (2-255)"			"$plca_node_count" \
	      "5. PLCA Burst Count (0x0-0xFF)"			"$plca_burst_count" \
	      "6. PLCA Burst Timer (0x0-0xFF)"			"$plca_burst_timer" \
	      "7. PLCA TO Timer (0x0-0xFF)"			"$plca_to_timer" \
	      2>&1 1>&3 `
	...
	3) #Edit
		...
		;;
	0) #Save
		data="SUBSYSTEM=="\"net\"", ACTION=="\"add\"", KERNEL=="\"$iface_name\"", "RUN+="\"/sbin/ethtool --set-plca-cfg $iface_name enable $plca_mode node-id $plca_node_id node-cnt $plca_node_count to-tmr $plca_to_timer burst-cnt $plca_burst_count burst-tmr $plca_burst_timer\""""
